...
@section('content')
    ...
    <div class="custom-card mt-15">
        <div class="card-header">
            ...
            @include('admin.components.link.custom',[
                'href'          => setRoute('admin.merchants.details',$user->username),
                'text'          => __("Profile"),
                'class'         => "btn btn--base",
                ...
                <div class="product-sales-btn">
                    @if ($user->kyc_verified != global_const()::VERIFIED)
                        @include('admin.components.button.custom',[
                            ...
                            'text'          => __("Approve"),
                            'permission'    => "admin.merchants.kyc.approve",
                        ])
                    @endif
                    ...
                    @if ($user->kyc_verified != global_const()::REJECTED)
                        @include('admin.components.button.custom',[
                            ...
                            'text'          => __("Reject"),
                            'permission'    => "admin.merchants.kyc.reject",
                        ])
                    @endif
                </div>
            @else
    ...
    @include('admin.components.modals.merchant-kyc-reject',compact("user"))
@endsection

@push('script')
    <script>
        $(".approve-btn").click(function(){
            var actionRoute = "{{ setRoute('admin.merchants.kyc.approve',$user->username) }}";
