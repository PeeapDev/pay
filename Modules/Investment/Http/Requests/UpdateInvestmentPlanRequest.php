...
class UpdateInvestmentPlanRequest extends FormRequest
{
    ...
    {
        if (request()->is_locked != 'Yes') {
            return [
                'name' => 'required|max:100|unique:investment_plans,name,' . $this->id,
                ...
                'interest_rate' => 'required|numeric|lt:amount|min:0|not_in:0',
                'interest_time_frame' => ['required', Rule::in(self::TIMEFRAME)],
                'capital_return_term' => ['required', Rule::in(['Term Basis', 'After Matured'])],
                'payment_methods' => 'required',
                'maximum_investors' => 'required|numeric|min:1|gte:maximum_limit_for_investor',
    ...
    public function fieldNames()
    {
        return [
            ...
            'maximum_limit_for_investor' => __('Maximum Limit For Investor'),
    ...
    public function message()
    {
        return [
            ...
            'name.max' => __('Name length should be maximum 100.'),
            'term.numeric' => __('Please enter any integer number.'),
            'currency_id.exists' => __('Currency does not exist.'),
